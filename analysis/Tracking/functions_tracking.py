...
def view_process_label_object(stack, threshold, y_res, x_res, nr_images):
    ...
    for idx in range(nr_images):
        print("Frame:", idx)
        blurred_image = cle.gaussian_blur(
            stack[idx], None, sigma_x=1, sigma_y=1, sigma_z=0
        )  # apply Gaussian blur to each image

        viewer.add_image(blurred_image, name="blurred", scale=(y_res, x_res), visible=False)  # add blurred images

        # threshold and label
        HP1a_greater_constant = threshold
        binary_cond = cle.greater_constant(blurred_image, None, HP1a_greater_constant)
        labeled_cond = cle.eroded_otsu_labeling(binary_cond, None, 2, 2)  # apply eroded otsu labeling
...
def tracking_object(stack, track_cost_cutoff, splitting_cost_cutoff, threshold, y_res, x_res, nr_images):
    ...
    track_df = track_df.reset_index()  # reset index in the dataframe
    ...
    track_df = track_df[["label", "centroid-0", "centroid-1", "frame", "area", "track_id"]]
